---
type: Operating System
title: tvOS
description: Apple operating system for Apple TV devices.
tags: [operating-system, apple, television]
status: draft
generated: { by: codex/gpt-5, at: "2026-07-26T00:00:00-04:00" }
schema_version: "0.1"
as_of: 2026-07-26
catalog_completeness: { level: inventory, note: "First-pass record; no claim is marked verified." }
field_dispositions:
  # BEGIN GENERATED TEXT EDITOR DISPOSITION
  - { field: text_editors, disposition: no-evidence-found, checked_at: 2026-07-26 }
  # END GENERATED TEXT EDITOR DISPOSITION
  - { field: development_origins, disposition: not-researched, checked_at: 2026-07-26 }
  - { field: lifecycle_events, disposition: not-researched, checked_at: 2026-07-26 }
  - { field: rights_regime, disposition: not-researched, checked_at: 2026-07-26 }
  - { field: licenses, disposition: no-evidence-found, checked_at: 2026-07-26 }
  - { field: programming_languages, disposition: not-researched, checked_at: 2026-07-26 }
  - { field: system_organization, disposition: not-researched, checked_at: 2026-07-26 }
  - { field: kernels, disposition: not-researched, checked_at: 2026-07-26 }
source_list: { title: "List of operating systems", revision: 1365063001, occurrences: [{ section: [Proprietary, Apple], label: tvOS, position: 46, target: "https://en.wikipedia.org/wiki/TvOS", depth: 4, parent_position: 45 }, { section: [Embedded, "Mobile operating systems"], label: tvOS, position: 818, target: "https://en.wikipedia.org/wiki/TvOS", depth: 2, parent_position: 816 }] }
discovery_provenance: [{ method: english-list, language: en, source: wikipedia-list, observed_at: 2026-07-26, disposition: included-system }]
names: [{ value: tvOS, kind: official, language: en, script: Latn, evidence: [apple-platforms], assertion_status: documented }]
organizations: [{ organization: Apple, roles: [developer, publisher], evidence: [apple-platforms], assertion_status: documented }]
countries_of_origin: [US]
design_purposes: [{ value: multimedia, primary: true, evidence: [apple-platforms], assertion_status: documented }]
development_status: { value: active, evidence: [apple-platforms], assertion_status: documented }
distribution_status: { value: public, evidence: [apple-platforms], assertion_status: documented }
gui_status: [{ value: first-party, evidence: [apple-platforms], assertion_status: provisional }]
interfaces: [{ name: "tvOS interface", style: graphical, modalities: [remote-control], provisioning: bundled, access: local-session, evidence: [apple-platforms], assertion_status: provisional }]
platforms: [{ value: "Apple TV", evidence: [apple-platforms], assertion_status: documented }]
sources:
  - { id: wikipedia-list, resource: "https://en.wikipedia.org/w/index.php?title=List_of_operating_systems&oldid=1365063001", title: "List of operating systems", source_kind: list }
  - { id: apple-platforms, resource: "https://developer.apple.com/platforms/", title: "Apple platforms", author: organization:Apple, source_kind: project-site }
# BEGIN GENERATED ENWIKI INFOBOX
first_pass_attributes:
  source: https://en.wikipedia.org/wiki/TvOS
  retrieved_at: '2026-07-26'
  assertion_status: provisional
  note: Raw discovery metadata from the linked English Wikipedia infobox; normalize
    and verify against stronger sources before marking verified.
  fields:
    developer: Apple
    country_of_origin: United States
    purpose: Television, casual gaming
    programming_languages: ASM C C++ Objective-C Swift
    first_release: January 9, 2007 ; 19 years ago ( 2007-01-09 ) as Apple TV Software
      October 29, 2015 ; 10 years ago ( 2015-10-29 ) as tvOS
    latest_release: 26.5 (May 11, 2026 ; 2 months ago ( 2026-05-11 ) ) [ ± ]
    last_updated: 26.5 (May 11, 2026 ; 2 months ago ( 2026-05-11 ) ) [ ± ]
    development_status: Current
    source_model: Closed , with open-source components
    os_family: Unix-like BSD
    gui: 10-foot user interface
    platforms: ARMv8-A (9.0–) ARMv7-A (4.0–7.9) x86 (1.0-3.0.2)
    kernel_type: Hybrid ( XNU )
    license: Commercial proprietary software
  country_evidence:
    assertion_status: inferred
    method: inferred-from-developer-country
    source: https://www.wikidata.org/wiki/Q312
# END GENERATED ENWIKI INFOBOX
# BEGIN GENERATED TEXT EDITORS
text_editor_research:
  inventory: /inventory/text-editor-associations.json
  checked_at: '2026-07-26'
  disposition: no-evidence-found
  note: No editor relationship was found in the linked Wikipedia page or direct Wikidata
    text-editor platform statements; primary manuals and distribution manifests still
    require research.
text_editors: []
# END GENERATED TEXT EDITORS
---

# tvOS

Apple identifies tvOS as an active Apple platform.[^apple-platforms]

[^apple-platforms]: [Apple platforms](https://developer.apple.com/platforms/)
